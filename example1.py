...
freq,xcorr = noise.noisecorr(tr1,tr2,window_length=1200.,overlap=0.5)
...
#                         freqmin=0.04,freqmax=0.25, min_vel=1.5, max_vel=5.0,min_amp=0.0,\
...
crossings,phase_vel = noise.get_smooth_pv(freq,smoothed,dist/1000.,ref_curve,\
                         freqmin=0.04,freqmax=0.25, min_vel=1.5, max_vel=5.5,\
                        filt_width=3,filt_height=0.2,x_overlap=0.75,y_overlap=0.75,pick_threshold=1.7,\
                       horizontal_polarization=False, smooth_spectrum=True
                       ,plotting=True)
...
plt.subplot(2,2,2)
plt.plot(crossings[:,0],crossings[:,1],'o',ms=1)
